...
public class XMemCachedClientPerformanceTest {
    private static MemcachedClient mcc;

    @Test
    public void testSingleThreadSet() {
        XMemSetThread memThread = new XMemSetThread(0, 1, null);
        memThread.run();
    }
    ...
    private void startSetThreads(int threadsNum) throws InterruptedException {
        ...
        for (int j = 0, jj = threadsNum; j < jj; ++j)
            ClientUtils.threadPool.submit(new XMemSetThread(j, threadsNum, countDownLatch));
    ...
    public static void setup() {
        mcc = ClientUtils.getXClient();
    }

    public static class XMemSetThread implements Runnable {
        private int index;
        private int threadsNum;
        private CountDownLatch countDownLatch;

        public XMemSetThread(int index, int threadsNum, CountDownLatch countDownLatch) {
            this.index = index;
        ...
        public void run() {
            try {
                for (int i = 0, ii = ClientUtils.MAX_OPERATION_NUM / threadsNum; i < ii; ++i) {
                    String key = index + ".x." + i;
                    if (!ClientUtils.setXCache(mcc, key, 600, key)) System.err.println("set " + key + " error!");
                }
            } finally {
    ...
    public void testSingleThreadGet() {
        XMemGetThread memThread = new XMemGetThread(0, 1, null);
        memThread.run();
    }
    ...
    private void startGetThreads(int threadsNum) throws InterruptedException {
        CountDownLatch countDownLatch = new CountDownLatch(threadsNum);
        for (int j = 0, jj = threadsNum; j < jj; ++j)
            ClientUtils.threadPool.submit(new XMemGetThread(j, threadsNum, countDownLatch));

        countDownLatch.await(2, TimeUnit.SECONDS);
    }

    public static class XMemGetThread implements Runnable {
        private int index;
        private int threadsNum;
        private CountDownLatch countDownLatch;

        public XMemGetThread(int index, int threadsNum, CountDownLatch countDownLatch) {
            this.index = index;
            this.threadsNum = threadsNum;
            this.countDownLatch = countDownLatch;
        }
        ...
        public void run() {
            try {
                for (int i = 0, ii = ClientUtils.MAX_OPERATION_NUM / threadsNum; i < ii; ++i) {
                    String key = index + ".x." + i;
                    String str = (String) ClientUtils.getXCache(mcc, key);
